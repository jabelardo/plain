...
package object text {
    ...
    if (null != array) {
      ...
      def dumpHex(i: Int) = {
        for (j ← 0 to 15 if i * 16 + j < length) {
          val pos = ((i * 16) + j)
          if (0 == pos % 16) buf.append("\n")
          buf.append((HEXCHAR((array(offset + pos) & 0x00f0) >> 4))).append((HEXCHAR(array(offset + pos) & 0x000f))).append(" ")
        }
        if (length / 16 == i) for (k ← 0 until (16 - (length % 16))) buf.append("   ")
      }
      def dumpChar(i: Int) = {
        buf.append("     ")
        for (j ← 0 to 15 if i * 16 + j < length) {
  ...
  final def hexDump(buffer: ByteBuffer, maxbytes: Int = 4096): String =
    if (buffer.hasArray)
      hexDump(buffer.array, buffer.position, scala.math.min(buffer.remaining, maxbytes))
    else {
      val a = new Array[Byte](buffer.position + buffer.remaining)
      val p = buffer.position
      buffer.get(a, p, buffer.remaining)
      buffer.position(p)
      hexDump(a, buffer.position, scala.math.min(buffer.remaining, maxbytes))
    }
