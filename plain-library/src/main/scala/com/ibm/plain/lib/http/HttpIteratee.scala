...
private object HttpConstants {
  ...
  @inline private[this] def b(in: Range.Inclusive): Set[Byte] = in.map(_.toByte).toSet
...
private object HttpIteratees {
  ...
  val readToken = for {
    token ← takeWhile(token)(UTF8)
  } yield token

  val readText = for {
    text ← takeWhile(text)(UTF8)
  } yield text
  ...
  val readRequestLine = {
    ...
    val readRequestUri: Iteratee[ByteBufferInput, (Seq[String], Option[String])] = {
      ...
      def readUriSegment(allowed: Set[Byte]): Iteratee[ByteBufferInput, String] = for {
        segment ← takeWhile(allowed)(UTF8)
      } yield codec.decode(segment)

      val readPath: Iteratee[ByteBufferInput, Seq[String]] = {

        def cont(segments: Seq[String]): Iteratee[ByteBufferInput, Seq[String]] = peek(1) >>> {
          case `/` ⇒ for {
            _ ← drop(1)
  ...
  val readRequestHeaders: Iteratee[ByteBufferInput, Seq[HttpHeader]] = {
    ...
    val readHeader: Iteratee[ByteBufferInput, HttpHeader] = {
      ...
      val readMultipleValues: Iteratee[ByteBufferInput, String] = null

      val readMultipleLines: Iteratee[ByteBufferInput, String] = null
    ...
    def cont(headers: Seq[HttpHeader]): Iteratee[ByteBufferInput, Seq[HttpHeader]] = peek(2) >>> {
      case "\r\n" ⇒ for {
        _ ← takeUntil(`\r\n`)
        done ← Done(headers)
      } yield done
...
object HttpTest extends App with HasLogger {
  ...
  def apply = try {
    ...
    for (_ ← 1 to 100000) infoNanoNanos(try {
