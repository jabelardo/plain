...
public final class Base64OutputStream extends FilterOutputStream {
	...
	public void write(final byte[] b, int off, final int len)
		...
		if (off + blen < end) {
			...
			out.write(Base64OutputStream.encode(b, off, blen, outbuf), 0,
					outlen);
			off += blen;
			count = 0;
		}
	...
	private void encode() throws IOException {
		...
		out.write(Base64OutputStream.encode(buffer, 0, bufsize, outbuf), 0,
				osize);
		// increment count
		count += osize;
	 ...
	 */
	private static byte[] encode(final byte[] inbuf, final int off, int size,
			byte[] outbuf) {
		if (outbuf == null) {
