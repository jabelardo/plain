 ...
 * @author John Mani
 * @author Bill Shannon
 * @author Guido Schmidt, IBM - renamed to Base64InputStream
 */
...
public final class Base64InputStream extends FilterInputStream {
	...
	/**
	 * Create a BASE64 decoder that decodes the specified input stream. The
	 ...
	 */
	public Base64InputStream(InputStream in, boolean ignoreErrors) {
		super(in);
	...
	/**
	 * The decoder algorithm. Most of the complexity here is dealing with error
	 * cases. Returns the number of bytes decoded, which may be zero. Decoding
	 ...
	 * @param pos
	 *            position in the buffer to start filling
	 * @param len
	 *            the number of bytes to fill
	 * @return the number of bytes filled, always a multiple of three, and may
	 *         be zero
	 * @exception IOException
	...
	private int decode(byte[] outbuf, int pos, int len) throws IOException {
		...
		while (len >= 3) {
			/*
			 * We need 4 valid base64 characters before we start decoding. We
			 * skip anything that"s not a valid base64 character (usually just
			 * CRLF).
			 */
			int got = 0;
			int val = 0;
