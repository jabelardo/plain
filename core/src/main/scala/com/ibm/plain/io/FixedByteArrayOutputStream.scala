...
final class FixedByteArrayOutputStream private (
 ...
 *
 */
object FixedByteArrayOutputStream {

  final def apply(array: Array[Byte], offset: Int, length: Int) = new FixedByteArrayOutputStream(array, offset, length)

  final def apply(array: Array[Byte]) = new FixedByteArrayOutputStream(array, 0, array.length)
